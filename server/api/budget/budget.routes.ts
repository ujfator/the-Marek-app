...
export class BudgetRoutes extends BaseController {
	...
	private _delete = (req: Request, res: Response): void => {
		this._budgetController
			.deleteBudget(req.params.budgetItemId)
			.then((resp: any) => {
				res.jsonp(resp);
			})
			.catch((e: any) => {
	...
	private _get = (req: Request, res: Response): void => {
		this._budgetController
			.getBudget(req.params.budgetItemId)
			.then((budgetItem: Budget) => {
